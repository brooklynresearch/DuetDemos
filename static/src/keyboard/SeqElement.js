...
class KeyboardElement extends events.EventEmitter {
	...
	resize(lowest, octaves){
		...
		// each of the keys
		const steps = 16
		const keyWidth = (1 / steps)
		const keyHeight = (1 / 12)
		...
		for(let i=0; i<16; i++){
			for(let j=0; j<12; j++){
				...
				node.style.width = `${keyWidth * 100}%`
				node.style.left  = `${keyWidth * (i * 100)}%`
				node.style.top  = `${keyHeight * (j * 100)}%`
				let nodeId = (j * steps) + i
				//let nodeId = j+48
				...
				this._bindKeyEvents(node)
				this._keys[j * steps + i] = node
				//this._keys[j] = node
			}
		}
		/*
	...
	_bindKeyEvents(key){
		...
		key.addEventListener('pointerover', (e) => {
			if (this._pointersDown[e.pointerId]){
				...
				//const noteNum = parseInt(e.target.id / 16) + 48
	...
	keyDown(noteNum, ai=false){
		...
		if (this._keys.hasOwnProperty(noteNum)){
			const seqNote = parseInt(noteNum/16) + 48
			const seqKey = this._keys[seqNote]
